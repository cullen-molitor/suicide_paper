...
save_folder <- "/home/tliddell/suicide_paper/data"
...
run_stagg_year_temp <- function(year) {  
  ...
  ## run stagg for temp
  temp_out <- staggregate_polynomial(clim_raster,
                                     polygon_weights,
                                     daily_agg = 'average',
                                     time_agg = 'month',
                                     degree = poly_order)
...
## save outputs
save_name <- paste0(paste("temp", "polynomial", years[1], years[length(years)], "EU", "ERA5",
